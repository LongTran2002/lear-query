...
function CabinRow({ cabin }) {
  ...
  const { isLoading, mutate: isDeleting } = useMutation({
    mutationFn: (id) => deleteCabins(id),
    onSuccess: () => {
      toast.success("Cabins has been deleted succesfully");
      querryClient.invalidateQueries({
        queryKey: ["cabins"],
      });
    },
    onError: (error) => {
      toast.error("Cannot deleted cabins");
    },
  });
  ...
  return (
    <TableRow role="row">
      ...
      <button onClick={() => isDeleting(cabinId)} disabled={isLoading}>
        Delete
      </button>
    </TableRow>
